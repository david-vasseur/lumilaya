...
type Props = { productId: string; productName: string; averageRating: number; reviewCount: number; };

export default function ReviewSummaryClient({ productId, productName, averageRating, reviewCount }: Props) {
  const { openModal } = useModalStore();
  const fullStars = Math.floor(averageRating);

  return (
    <div className="flex items-center gap-4 mb-6">
      <Link href={`/avis/${encodeURIComponent(productName)}`} className="flex items-center gap-1 text-[#2C2C2C] hover:underline">
        {[...Array(5)].map((_, i) => (
